...
> **待用户拍板的冲突**: 报告 P0-5 第 3 条建议 `category=convention`(风格/约定)降为"建议级、
> 不计门禁", 而本仓 `docs/severity-1to5-design.md` 的策略锚点是"明确约定违反 = 3(必修不阻塞)"。
> 本 PR **不动这条锚点**(改它会影响既有判定与多处测试), 只按上面的 P0-5 规则处理
> "工具链已覆盖"的情形。要不要把 convention 整体降到 2, 由用户决定。
